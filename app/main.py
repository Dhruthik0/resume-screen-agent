...
# Optional: thresholds for agent classification
...
if st.button("🚀 Run Screening Agent"):
    ...
    # Save to session_state so they survive future reruns (checkbox, sliders, etc.)
...
if st.session_state.has_run and st.session_state.sorted_results is not None:
    ...
    if df is not None:
        # Show expandable cards with explanations
        for r in sorted_results:
            score = r.get("final_score", 0)
    ...
    # --- HR email draft (only available AFTER run, controlled by checkbox) ---
